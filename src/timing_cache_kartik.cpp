...
class HitEvent : public TimingEvent {
    private:
        TimingCacheKartik* cache;

    public:
        HitEvent(TimingCacheKartik* _cache,  uint32_t postDelay, int32_t domain) : TimingEvent(0, postDelay, domain), cache(_cache) {}
...
class MissStartEvent : public TimingEvent {
    ...
    public:
        ...
        MissStartEvent(TimingCacheKartik* _cache,  uint32_t postDelay, int32_t domain) : TimingEvent(0, postDelay, domain), cache(_cache) {}
...
class MissResponseEvent : public TimingEvent {
    private:
        ...
        MissStartEvent* mse;
    public:
        MissResponseEvent(TimingCacheKartik* _cache, MissStartEvent* _mse, int32_t domain) : TimingEvent(0, 0, domain), cache(_cache), mse(_mse) {}
...
class MissWritebackEvent : public TimingEvent {
    private:
        ...
        MissStartEvent* mse;
    public:
        MissWritebackEvent(TimingCacheKartik* _cache,  MissStartEvent* _mse, uint32_t postDelay, int32_t domain) : TimingEvent(0, postDelay, domain), cache(_cache), mse(_mse) {}
...
class ReplAccessEvent : public TimingEvent {
    ...
    public:
        uint32_t accsLeft;
        ReplAccessEvent(TimingCacheKartik* _cache, uint32_t _accsLeft, uint32_t preDelay, uint32_t postDelay, int32_t domain) : TimingEvent(preDelay, postDelay, domain), cache(_cache), accsLeft(_accsLeft) {}
        void simulate(uint64_t startCycle) {cache->simulateReplAccess(this, startCycle);}
};

TimingCacheKartik::TimingCacheKartik(uint32_t _numLines, CC* _cc, CacheArray* _array, ReplPolicy* _rp,
...
void TimingCacheKartik::initStats(AggregateStat* parentStat) {
...
uint64_t TimingCacheKartik::access(MemReq& req) {
 ...
 * cycle in advance.
 */
uint64_t TimingCacheKartik::tryLowPrioAccess(uint64_t cycle) {
...
void TimingCacheKartik::simulateHit(HitEvent* ev, uint64_t cycle) {
...
void TimingCacheKartik::simulateMissResponse(MissResponseEvent* ev, uint64_t cycle, MissStartEvent* mse) {
    profMissRespLat.inc(cycle - mse->startCycle);
    ev->done(cycle);
}

void TimingCacheKartik::simulateMissWriteback(MissWritebackEvent* ev, uint64_t cycle, MissStartEvent* mse) {
...
void TimingCacheKartik::simulateReplAccess(ReplAccessEvent* ev, uint64_t cycle) {
    assert(ev->accsLeft);
    uint64_t lookupCycle = tryLowPrioAccess(cycle);
    if (lookupCycle) {
        ev->accsLeft--;
