...
bool ContentionSim::CompareEvents::operator()(TimingEvent* lhs,
                                              TimingEvent* rhs) const {
  return lhs->cycle > rhs->cycle;
}

bool ContentionSim::CompareDomains::operator()(DomainData* d1,
                                               DomainData* d2) const {
...
ContentionSim::ContentionSim(uint32_t _numDomains, uint32_t _numSimThreads) {
  ...
  if ((numDomains % numSimThreads) != 0)
    panic("numDomains(%d) must be a multiple of numSimThreads(%d) for now",
  ...
  for (uint32_t i = 0; i < numSimThreads; i++) {
    ...
    futex_lock(
        &simThreads[i]
             .wakeLock);  // starts locked, so first actual call to lock blocks
  ...
  lastCrossing = gm_calloc<CrossingEventInfo>(
      ...
      MAX_THREADS);  // TODO: refine... this allocs too much
}
...
void ContentionSim::initStats(AggregateStat* parentStat) {
  ...
  for (uint32_t i = 0; i < numDomains; i++) {
    ...
    domains[i].profIncomingCrossings.init("ixe", "Incoming crossing events",
                                          numDomains);
    domains[i].profIncomingCrossingSims.init(
        "ixs", "Incoming crossings simulated but held", numDomains);
    domains[i].profIncomingCrossingHist.init(
        "ixh", "Incoming crossings held count histogram", 33 /*32 means >31*/);
...
/**
 * «Simulates function phase.
 * «Happens at the end of each phase of cyles,
 * whose duration is defined in the config file»
 * @param[in] limit  «limit-explanations»
 *
 * «@throw »
 */
...
/**
 * Enqueues objects into the priority queue
...
void ContentionSim::enqueue(TimingEvent* ev, uint64_t cycle) {
  assert(inCSim);
  assert(ev);
  assert_msg(cycle >= lastLimit,
             "Enqueued event before last limit! cycle %ld min %ld", cycle,
             lastLimit);
  // Hacky, but helpful to chase events scheduled too far ahead due to bugs
  // (e.g., cycle -1). We should probably formalize this a bit more
  assert_msg(cycle < lastLimit + 10 * zinfo->phaseLength + 1000000,
             "Queued event too far into the future, cycle %ld lastLimit %ld",
             cycle, lastLimit);

  assert_msg(cycle >= domains[ev->domain].curCycle,
             "Queued event goes back in time, cycle %ld curCycle %ld", cycle,
...
void ContentionSim::enqueueSynced(TimingEvent* ev, uint64_t cycle) {
  ...
  assert_msg(cycle >= lastLimit,
             "Enqueued (synced) event before last limit! cycle %ld min %ld",
             cycle, lastLimit);
  ...
  assert_msg(
      cycle < lastLimit + 10 * zinfo->phaseLength + 10000,
      "Queued  (synced) event too far into the future, cycle %ld lastLimit %ld",
      cycle, lastLimit);
...
void ContentionSim::enqueueCrossing(CrossingEvent* ev, uint64_t cycle,
                                    uint32_t srcId, uint32_t srcDomain,
  ...
  if (!isFirst) {
    ...
    if (b->srcDomain == (uint32_t)ev->domain &&
        (uint32_t)b->domain == ev->srcDomain) {
      isResp = true;
      req = b;
    }
  }
  ...
  if (isResp) {
    ...
    CrossingEventInfo* last =
        &lastCrossing[(srcId * numDomains + srcDomain) * numDomains +
                      dstDomain];
    uint64_t srcDomCycle = domains[srcDomain].curCycle;
    if (last->cycle > srcDomCycle &&
        last->cycle <= cycle) {  // NOTE: With the OOO model, last->cycle >
                                 // cycle is now possible, since requests are
                                 // issued in instruction order -> ooo
      // Chain to previous req
      assert_msg(last->cycle <= cycle, "last->cycle (%ld) > cycle (%ld)",
                 last->cycle, cycle);
      last->ev->addChild(ev, evRec);
    } else {
      // We can't queue --- queue directly (synced, we're in phase 1)
      assert(cycle >= srcDomCycle);
      // info("Queuing xing %ld %ld (lst eve too old at cycle %ld)", cycle,
      // srcDomCycle, last->cycle);
...
/**
 * This is simulatePhaseThread function
 * It gets a reference to the domain number of threads
 * Subseqently it gets a reference to number of domains
 *
 *
 *
 *
...
void ContentionSim::simulatePhaseThread(uint32_t thid) {
  uint32_t thDomains =
      simThreads[thid].supDomain - simThreads[thid].firstDomain;
  ...
  if (thDomains == 1) {
    ...
    if (limit % 10000000 == 0) {
      ...
      for (std::pair<uint64_t, TimingEvent*> p : simThreads[thid].logVec) {
        ...
        if (desc == "") {  // non-existnt
          ...
          if (ce) {
            ss << " slack " << (ce->preSlack + ce->postSlack) << " osc "
               << ce->origStartCycle << " cnt " << ce->simCount;
          }
    ...
    std::priority_queue<DomainData*, std::vector<DomainData*>, CompareDomains>
        domPq;
    for (uint32_t i = simThreads[thid].firstDomain;
         i < simThreads[thid].supDomain; i++) {
    ...
    while (numFinished < thDomains) {
      while (domPq.size()) {
        ...
        if (!pq.size() || pq.firstCycle() > limit) {
          ...
          // info("YYY %d %ld %ld %d", numFinished, domPq.size(),
          // domain->curCycle, domain->prio);
          uint64_t cycle;
          TimingEvent* te = pq.dequeue(cycle);

          //**** debug the priority queue ****//
          // info("Event has min start cycle %d\n",(int)te->getMinStartCycle());
          // //print out the stuff in the pq
          //**** end of prio queue debug ****//
          // uint64_t Cycle = pq.size()? pq.firstCycle() : cycle;
      ...
      while (stalledQueue.size()) {
        ...
        if (!pq.size() || pq.firstCycle() > limit) {
          ...
          // info("SSS %d %ld %ld", numFinished, .size(), domain->curCycle);
          uint64_t cycle;
          TimingEvent* te = pq.dequeue(cycle);
