 ...
 * potentially O(n^2)), and in practice
...
// op.
// Predication is rare enough that we don't need to model it perfectly to be
...
inline void OOOCore::bbl(Address bblAddr, BblInfo* bblInfo) {
  ...
  if (branchPc && !branchPred.predict(branchPc, branchTaken)) {
    ...
    for (uint32_t i = 0; i < 5 * 64 / lineSize; i++) {
      uint64_t fetchLat =
          l1i->load(wrongPathAddr + lineSize * i, curCycle) - curCycle;
...
void OOOCore::BblFunc(THREADID tid, ADDRINT bblAddr, BblInfo* bblInfo) {
  ...
  while (core->curCycle > core->phaseEndCycle) {
   ...
   //info ("Instructions at the end of this phase is %d, phase length is %d", (int)core->instrs, (int)zinfo->phaseLength);
    ...
    // by some other thread. If TakeBarrier context-switches us,
    // the *only* safe option is to return inmmediately after we detect this, or
    ...
    // This is fine, since the loop looks at core values directly and there are
    // no locals involved,
