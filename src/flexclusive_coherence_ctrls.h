...
#ifndef FLEXCLUSIVE_COHERENCE_CTRLS_H_
#define FLEXCLUSIVE_COHERENCE_CTRLS_H_
 ...
 * respect to the upper level and issues
 *    requests (accesses) to upper levels.
 *  - The TOP controller, which keeps state of lines w.r.t. lower levels of the
 * hierarchy (e.g. sharer lists),
 *    and issues requests (invalidates) to lower levels.
 * The naming scheme is PROTOCOL-CENTRIC, i.e. if you draw a multi-level
 * hierarchy, between each pair of levels
 * there is a top CC at the top and a bottom CC at the bottom. Unfortunately, if
 * you look at the caches, the
 * bottom CC is at the top is at the bottom. So the cache class may seem a bit
 * weird at times, but the controller
...
class flexclusive_MESIBottomCC : public GlobAlloc {
 private:
  MESIState* array;
 ...
 public:
  flexclusive_MESIBottomCC(uint32_t _numLines, uint32_t _selfId,
                           bool _nonInclusiveHack)
      : numLines(_numLines),
        selfId(_selfId),
...
// and invalidates
class flexclusive_MESITopCC : public GlobAlloc {
 private:
  struct Entry {
    uint32_t numSharers;
 ...
 public:
  flexclusive_MESITopCC(uint32_t _numLines, bool _nonInclusiveHack)
...
static inline bool flexclusive_CheckForMESIRace(AccessType& type,
                                                MESIState* state,
...
class flexclusive_MESICC : public CC {
 private:
  flexclusive_MESITopCC* tcc;
  flexclusive_MESIBottomCC* bcc;
  uint32_t numLines;
  bool nonInclusiveHack;
  g_string name;
 ...
 public:
  // Initialization
  flexclusive_MESICC(uint32_t _numLines, bool _nonInclusiveHack,
                     g_string& _name)
      : tcc(nullptr),
        bcc(nullptr),
        numLines(_numLines),
  ...
  void setParents(uint32_t childId, const g_vector<MemObject*>& parents,
                  Network* network) {
    bcc = new flexclusive_MESIBottomCC(numLines, childId, nonInclusiveHack);
    bcc->init(parents, network, name.c_str());
  }

  void setChildren(const g_vector<BaseCache*>& children, Network* network) {
    tcc = new flexclusive_MESITopCC(numLines, nonInclusiveHack);
  ...
  bool startAccess(MemReq& req) {
     ...
     * skip the access.
     */
    bool skipAccess = flexclusive_CheckForMESIRace(req.type /*may change*/,
    ...
    if (cs == EX) {
      if (lineId == -1) {
        ...
        respCycle = bcc->processAccess(req.lineAddr, lineId, req.type,
                                       startCycle, req.srcId, req.flags, cs);
        bool lowerLevelWriteback = false;
        ...
        respCycle = bcc->processAccess(req.lineAddr, lineId, req.type,
                                       startCycle, req.srcId, flags, cs);
        if (getDoneCycle) *getDoneCycle = respCycle;
        if (!isPrefetch) {  // prefetches only touch bcc; the demand request
                            // from the core will pull the line to lower level
          ...
          respCycle = tcc->processAccess(
              req.lineAddr, lineId, req.type, req.childId, true, req.state,
              &lowerLevelWriteback, respCycle, req.srcId, flags, false, cs);
        }
      }
      return respCycle;
    } else {
      if (lineId == -1 ||
        ...
        if (!isPrefetch) {  // prefetches only touch bcc; the demand request
          ...
          respCycle = tcc->processAccess(
              ...
              respCycle, req.srcId, flags, bcc->isValid(lineId), cs);
          if (lowerLevelWriteback) {
            // Essentially, if tcc induced a writeback, bcc may need to do an
            // E->M transition to reflect that the cache now has dirty data
...
class flexclusive_MESITerminalCC : public CC {
 private:
  flexclusive_MESIBottomCC* bcc;
  uint32_t numLines;
  g_string name;
 ...
 public:
  // Initialization
  flexclusive_MESITerminalCC(uint32_t _numLines, const g_string& _name)
      : bcc(nullptr), numLines(_numLines), name(_name) {}

  void setParents(uint32_t childId, const g_vector<MemObject*>& parents,
                  Network* network) {
    bcc = new flexclusive_MESIBottomCC(numLines, childId, false /*inclusive*/);
    bcc->init(parents, network, name.c_str());
  }

  void setChildren(const g_vector<BaseCache*>& children, Network* network) {
    panic(
        "[%s] flexclusive_MESITerminalCC::setChildren cannot be called -- "
        "terminal caches cannot have children!",
        name.c_str());
  }
  ...
  uint64_t processAccess(const MemReq& req, int32_t lineId, uint64_t startCycle,
    ...
    assert(!getDoneCycle);
    // if needed, fetch line or upgrade miss from upper level
    uint64_t respCycle = bcc->processAccess(
        req.lineAddr, lineId, req.type, startCycle, req.srcId, req.flags, cs);
    // at this point, the line is in a good state w.r.t. upper levels
    return respCycle;
  }
...
#endif  // FLEXCLUSIVE_COHERENCE_CTRLS_H_
