 ...
 * array,
 * and then go through the normal access path. Because there is one line per
 * set,
...
class FilterCache : public Cache {
 ...
 public:
  ...
  inline uint64_t load(Address vAddr, uint64_t curCycle) {
    ...
    uint64_t availCycle = filterArray[idx].availCycle;  // read before, careful
                                                        // with ordering to
                                                        // avoid timing races
  ...
  uint64_t replace(Address vLineAddr, uint32_t idx, bool isLoad,
    ...
    MemReq req = {pLineAddr, isLoad ? GETS : GETX, 0,          &dummyState,
                  curCycle,  &filterLock,          dummyState, srcId,
                  reqFlags};
    ...
    //(e.g., st to x, ld from x+8) and we implement store-load forwarding at the
    //core.
    // So if this is a load, it always sets availCycle; if it is a store hit, it
    // doesn't
