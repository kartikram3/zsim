...
class ReplPolicy : public GlobAlloc {
 ...
 public:
  ...
  virtual void setCacheArray(CacheArray* cache_array){};
};
 ...
 * transparently to each type of array
 ...
 * indicating the line that will be inserted;
 *   then it calls recordCandidate() for each candidate it finds; finally, it
 * calls getBestCandidate() to get the
 *   line chosen for eviction. When the replacement is done, replaced() is
 * called. The division of getBestCandidate()
 *   and replaced() happens because the former is called in preinsert(), and the
 * latter in postinsert(). Note how the
 *   same restrictions on concurrent insertions extend to this class, i.e.
 * startReplacement()/recordCandidate()/
 *   getBestCandidate() will be atomic, but there may be intervening update()
 * calls between getBestCandidate() and
 *   replaced().
 */
...
class LRUReplPolicy : public ReplPolicy {
 ...
 private:
  inline uint64_t score(uint32_t id) {  // higher is least evictable
    ...
    // (2) sharers, and
    // (3) timestamp
    return (sharersAware ? cc->numSharers(id) + 1 : 0) * timestamp *
               cc->isValid(id) +
...
class TreeLRUReplPolicy : public LRUReplPolicy<true> {
 ...
 public:
  ...
  uint32_t getBestCandidate() {
    ...
    // array[candArray[i]]);
    // info(" res: %d (%d %ld)", start, candArray[start],
...
template <bool sharersAware>
class CLU_LRU_ReplPolicy : public ReplPolicy {
 protected:
  uint64_t timestamp;  // incremented on each access
  uint64_t* array;
  uint32_t numLines;
 ...
 public:
  explicit CLU_LRU_ReplPolicy(uint32_t _numLines)
      : timestamp(1), numLines(_numLines) {
    array = gm_calloc<uint64_t>(numLines);
  }

  ~CLU_LRU_ReplPolicy() { gm_free(array); }

  void update(uint32_t id, const MemReq* req) { array[id] = timestamp++; }

  void replaced(uint32_t id) { array[id] = 0; }

  void setCacheArray(CacheArray* cache_array) {
  ...
  inline uint32_t rank(const MemReq* req, C cands) {
    uint32_t bestCand = -1;
    uint32_t bestnonDupCand = -1;
    uint64_t bestScore = (uint64_t)-1L;
    for (auto ci = cands.begin(); ci != cands.end(); ci.inc()) {
      uint32_t s = score(*ci);
      bestCand = (s < bestScore) ? *ci : bestCand;
      if (cache_array->getDup(bestCand) == DUP) {
        bestnonDupCand = bestCand;
      }
      bestScore = MIN(s, bestScore);
    }

    if (bestnonDupCand != -1) return bestnonDupCand;
...
/* Plain ol' LRU with PacmanM, though this one is sharers-aware, prioritizing
 * lines that have
 * sharers down in the hierarchy vs lines not shared by anyone.
 */
template <bool sharersAware>
class LRURepl_pacmanM_Policy : public ReplPolicy {
 protected:
  uint64_t timestamp;  // incremented on each access
  uint64_t* array;
  uint32_t numLines;
 ...
 public:
  explicit LRURepl_pacmanM_Policy(uint32_t _numLines)
      : timestamp(1), numLines(_numLines) {
    array = gm_calloc<uint64_t>(numLines);
  }

  ~LRURepl_pacmanM_Policy() { gm_free(array); }
  ...
  inline uint32_t rank(const MemReq* req, C cands) {
    uint32_t bestCand = -1;
    uint64_t bestScore = (uint64_t)-1L;
    uint64_t bestPrefetchCand = -1;
    for (auto ci = cands.begin(); ci != cands.end(); ci.inc()) {
      uint32_t s = score(*ci);
      bestCand = (s < bestScore) ? *ci : bestCand;
      if (cache_array->getPrefetch(bestCand) == true) {
        bestPrefetchCand = bestCand;
      }
      bestScore = MIN(s, bestScore);
    }

    if (bestPrefetchCand != -1)
      return bestPrefetchCand;
    else
      return bestCand;
  }
...
template <bool sharersAware>
class DRRIPPolicy : public ReplPolicy {
 protected:
  uint64_t timestamp;  // incremented on each access
  uint64_t* array;
  uint32_t numLines;
 ...
 public:
  explicit DRRIPPolicy(uint32_t _numLines) : timestamp(1), numLines(_numLines) {
    array = gm_calloc<uint64_t>(numLines);
    repl_state = gm_calloc<DRRIP>(numLines);
  }

  ~DRRIPPolicy() { gm_free(array); }
  ...
  inline uint32_t rank(const MemReq* req, C cands) {
    ...
    int flag = 0;  //the states are defined in memory_hierarchy.h
    int cur_id;
    while (flag == 0) {
      for (auto ci = cands.begin(); ci != cands.end(); ci.inc()) {
        int id = *ci;
        ...
        cur_state = cur_state == 3 ? cur_state : cur_state + 1;
        if (cur_state == 3) {
          cur_id = id;
          flag = 1;
        }  // select one of the blocks
      }
      if (flag == 0) {  // increment states of everyone
        for (auto ci = cands.begin(); ci != cands.end(); ci.inc()) {
          int id = *ci;
          switch (repl_state[id]) {
            ...
            case D:
              panic(
                  "Incrementing states although one is already at distant "
                  "state");
              break;
            default:;
          }
