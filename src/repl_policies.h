...
class LRUReplPolicy : public ReplPolicy {
    ...
    private:
        inline uint64_t score(uint32_t id) { //higher is least evictable
            ...
            // (2) sharers, and
            // (3) timestamp
            return (sharersAware? cc->numSharers(id)+1 : 0)*timestamp*cc->isValid(id) + array[id]*cc->isValid(id);
        }
};
...
class CLU_LRU_ReplPolicy : public ReplPolicy {
    ...
    public:
        explicit CLU_LRU_ReplPolicy(uint32_t _numLines) : timestamp(1), numLines(_numLines) {
            array = gm_calloc<uint64_t>(numLines);
        }
...
/* Plain ol' LRU with PacmanM, though this one is sharers-aware, prioritizing lines that have
 * sharers down in the hierarchy vs lines not shared by anyone.
 */
template <bool sharersAware>
class LRURepl_pacmanM_Policy : public ReplPolicy {
    ...
    public:
        explicit LRURepl_pacmanM_Policy(uint32_t _numLines) : timestamp(1), numLines(_numLines) {
            array = gm_calloc<uint64_t>(numLines);
        }

        ~LRURepl_pacmanM_Policy() {
            gm_free(array);
        }
