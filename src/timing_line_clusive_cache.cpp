...
class HitEvent : public TimingEvent {
 private:
  timing_line_clusive_cache* cache;

 public:
  HitEvent(timing_line_clusive_cache* _cache, uint32_t postDelay,
           int32_t domain)
...
class MissStartEvent : public TimingEvent {
 ...
 public:
  ...
  MissStartEvent(timing_line_clusive_cache* _cache, uint32_t postDelay,
                 int32_t domain)
      : TimingEvent(0, postDelay, domain), cache(_cache) {}
...
class MissResponseEvent : public TimingEvent {
 ...
 public:
  MissResponseEvent(timing_line_clusive_cache* _cache, MissStartEvent* _mse,
                    int32_t domain)
...
class MissWritebackEvent : public TimingEvent {
 ...
 public:
  MissWritebackEvent(timing_line_clusive_cache* _cache, MissStartEvent* _mse,
...
class ReplAccessEvent : public TimingEvent {
 ...
 public:
  uint32_t accsLeft;
  ReplAccessEvent(timing_line_clusive_cache* _cache, uint32_t _accsLeft,
...
timing_line_clusive_cache::timing_line_clusive_cache(
    uint32_t _numLines, CC* _cc, CacheArray* _array, ReplPolicy* _rp,
    uint32_t _accLat, uint32_t _invLat, uint32_t mshrs, uint32_t _tagLat,
    uint32_t _ways, uint32_t _cands, uint32_t _domain, const g_string& _name)
...
void timing_line_clusive_cache::initStats(AggregateStat* parentStat) {
  ...
  ExclWB.init ("ExclWB", "WB due to exclusive  lines");
  profDup.init ("profDup", "Duplicate line count");
  profNoDup.init("profNoDup", "Non duplicate line count");
  cacheStat->append(&profOccHist);
  cacheStat->append(&ExclWB);
  cacheStat->append(&profDup);
  cacheStat->append(&profNoDup);
...
void timing_line_clusive_cache::setasLLC() {  // invoked at cache creation only

  Cache::setLLCflag();  // access parent cache llc set function
}

uint64_t timing_line_clusive_cache::access(MemReq& req) {
  ...
  if (likely(!skipAccess)) {
    ...
    if (lineId == -1) {
      ...
      if ((req.type == GETS || req.type == GETX)) {
        ...
        evDoneCycle = cc->processEviction(req, wbLineAddr, lineId, respCycle);
      ...
      if (lineId != -1) {
        ...
        if (req.type == GETS || req.type == GETX){
          if (req.flags & MemReq::INNER_COPY){
                assert(array->getDup(lineId) == DUP);
          }else{
              assert(array->getDup(lineId) == noDUP);
              array->setDup(lineId); //duplicate on hit
              profDup.inc();
              profNoDup.inc(-1);
          }
        }else{
          if (req.flags & MemReq::INNER_COPY){
                ...
                array->setnoDup(lineId); //it is a PUTX hit, so noDupp
                profNoDup.inc();
          }
        }
    }
    ...
    respCycle = cc->processAccess(req, lineId, respCycle, &getDoneCycle);
      ...
      if (lineId != -1){  //means line was allocated, or hit
...
uint64_t timing_line_clusive_cache::highPrioAccess(uint64_t cycle) {
 ...
 * cycle in advance.
 */
uint64_t timing_line_clusive_cache::tryLowPrioAccess(uint64_t cycle) {
...
void timing_line_clusive_cache::simulateHit(HitEvent* ev, uint64_t cycle) {
...
void timing_line_clusive_cache::simulateMissStart(MissStartEvent* ev,
                                                  uint64_t cycle) {
  if (activeMisses < numMSHRs) {
    activeMisses++;
...
void timing_line_clusive_cache::simulateMissResponse(MissResponseEvent* ev,
                                                     uint64_t cycle,
                                                     MissStartEvent* mse) {
...
void timing_line_clusive_cache::simulateMissWriteback(MissWritebackEvent* ev,
                                                      uint64_t cycle,
                                                      MissStartEvent* mse) {
...
void timing_line_clusive_cache::simulateReplAccess(ReplAccessEvent* ev,
                                                   uint64_t cycle) {
  assert(ev->accsLeft);
  uint64_t lookupCycle = tryLowPrioAccess(cycle);
  if (lookupCycle) {
