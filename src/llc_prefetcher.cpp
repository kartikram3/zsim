...
void llc_StreamPrefetcher::setParents(uint32_t _childId,
...
void llc_StreamPrefetcher::setChildren(const g_vector<BaseCache*>& children,
                                   Network* network) {
...
void llc_StreamPrefetcher::initStats(AggregateStat* parentStat) {
...
uint64_t llc_StreamPrefetcher::access(MemReq& req) {
    ...
    if (e.stride == stride) {
      ...
      if (e.conf.pred()) {  // do prefetches
        ...
        if (prefetchPos < 64 && !e.valid[prefetchPos]) {
          MemReq::Flag f = (MemReq::Flag)(MemReq::PREFETCH | MemReq::LLC_PREFETCH) ;
          if (tr.startEvent == nullptr) {
            f = (MemReq::Flag)(MemReq::DONT_RECORD | MemReq::PREFETCH | MemReq::LLC_PREFETCH );
          }
...
uint64_t llc_StreamPrefetcher::invalidate(const InvReq& req) {
