...
class HitEvent : public TimingEvent {
 ...
 public:
  HitEvent(non_inclusive_cache_timing* _cache, uint32_t postDelay,
           int32_t domain)
...
class MissStartEvent : public TimingEvent {
 ...
 public:
  ...
  MissStartEvent(non_inclusive_cache_timing* _cache, uint32_t postDelay,
                 int32_t domain)
      : TimingEvent(0, postDelay, domain), cache(_cache) {}
...
class MissResponseEvent : public TimingEvent {
 ...
 public:
  MissResponseEvent(non_inclusive_cache_timing* _cache, MissStartEvent* _mse,
                    int32_t domain)
...
class MissWritebackEvent : public TimingEvent {
 ...
 public:
  MissWritebackEvent(non_inclusive_cache_timing* _cache, MissStartEvent* _mse,
...
class ReplAccessEvent : public TimingEvent {
 ...
 public:
  uint32_t accsLeft;
  ReplAccessEvent(non_inclusive_cache_timing* _cache, uint32_t _accsLeft,
...
non_inclusive_cache_timing::non_inclusive_cache_timing(
...
uint64_t non_inclusive_cache_timing::access(MemReq& req) {
  ...
  // add statistic for PUTS of lines which are already there
  // means was correctly non-inclusive
  ...
  if (likely(!skipAccess)) {
    ...
    if (lineId == -1 &&
        cc->shouldAllocate(req)) {  // need to add inner level search
      // when miss at llc
      ...
      uint64_t lifetime =
          ...
          (float)(zinfo->phaseLength * zinfo->statsPhaseInterval);  // histogram
                                                                    // partition
                                                                    // on cycles
                                                                    // per dump
      ...
      if (lifetime >
          99) {  // lump greater than dump length lifetimes in the same category
        lifetime = 99;
      }

      phase_lifetimes.inc(
          lifetime);  // approximate histogram of lifetimes of lines in a phase
      agg_lifetimes.inc(
          lifetime);  // approximate histogram of lifetimes of lines in a phase
      ...
      if (agg_hits[lineId] >=
          1) {  // so we would be better off if this line was excl
        should_be_exclusive_lines.inc();
        uint64_t fetch_cycle = array->getFetchTime(lineId);
        excl_interval.inc(respCycle - fetch_cycle);
      }

      array->setFetchTime(respCycle, lineId);
    ...
    if (lineId != -1) {  // no need to record in case of non inclusive wb
      ...
      // if (getDoneCycle - req.cycle == accLat) { //does not work with l3
      // prefetcher
      if (hitflag) {
        // Hit
        ...
        // respCycle); // does not work with l3 prefetcher
...
void non_inclusive_cache_timing::simulateMissStart(MissStartEvent* ev,
                                                   uint64_t cycle) {
  if (activeMisses < numMSHRs) {
    activeMisses++;
...
void non_inclusive_cache_timing::simulateMissResponse(MissResponseEvent* ev,
                                                      uint64_t cycle,
                                                      MissStartEvent* mse) {
...
void non_inclusive_cache_timing::simulateMissWriteback(MissWritebackEvent* ev,
                                                       uint64_t cycle,
                                                       MissStartEvent* mse) {
...
void non_inclusive_cache_timing::simulateReplAccess(ReplAccessEvent* ev,
                                                    uint64_t cycle) {
  assert(ev->accsLeft);
  uint64_t lookupCycle = tryLowPrioAccess(cycle);
  if (lookupCycle) {
