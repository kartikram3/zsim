...
#ifndef TIMING_FLEXCLUSIVE_CACHE_ 
#define TIMING_FLEXCLUSIVE CACHE_
...
class timing_flexclusive_cache : public Cache {
 private:
  ...
  //More stats added by Kartik
  Counter agg_NI_accesses;
  Counter agg_NI_hits;
  Counter agg_EX_accesses;
  Counter agg_EX_hits;
 ...
 public:
  ...
  timing_flexclusive_cache(uint32_t _numLines, CC* _cc, CacheArray* _array, ReplPolicy* _rp,
              uint32_t _accLat, uint32_t _invLat, uint32_t mshrs,
              uint32_t tagLat, uint32_t ways, uint32_t cands, uint32_t _domain,
